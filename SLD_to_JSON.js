...
function writeStartOfJSON() {
  var top =
    ...
    '": { "type": "vector", "url": "' +
    sourceUrl +
    '" } }, "glyphs": "mapbox://fontstack/{fontstack}/{range}.pbf", "sprite": "https://www.mapbox.com/mapbox-gl-styles/sprites/sprite", "layers": [ { "id": "background", "type": "background", "paint": { "background-color": "rgb(237, 234, 235)" } }';
...
var parseFile = function(data, file) {
  ...
  parser.parseString(data, function(err, result) {
    var FeatureTypeStyle =
      result.StyledLayerDescriptor["NamedLayer"][0].UserStyle[0][
        "se:FeatureTypeStyle"
      ];
    ...
    var k;
    var rules = [];
    for (k = 0; k < FeatureTypeStyle.length; k++) {
      //some files had more than one FeatureTypeStyle
...
function writeJSON(symbTag, type, name, minzoom, maxzoom, file) {
  ...
  try {
    ...
    if (
      cssObj["fill-color"] !== undefined &&
      cssObj["line-color"] !== undefined
    ) {
      ...
      // Add to the layers
      layers.push(styleObj1);
      layers.push(styleObj2);
...
function getSymbolizersObj(symbTag, type, file) {
  ...
  var i;
  var cssObj = {};
  for (i = 0; i < Object.keys(symbTag[0]).length; i++) {
    //for all tags under <-Symbolizer>
    var tagName = Object.keys(symbTag[0])[i];
    if (VALID_ATTR_TAGS.indexOf(tagName) > -1) {
      //if tag exists in valid-array, eks Stroke

      //if values are not in the regular place
      if (
        DIFF_ATTR_TAG.indexOf(tagName) > -1 ||
        (tagName === "se:Fill" &&
          symbTag[0]["se:Fill"][0]["se:GraphicFill"] !== undefined)
      ) {
...
function getCssParameters(symbTag, validAttrTag, type, outerTag) {
  ...
  if (outerTag === undefined) {
    ...
    var allCssArray =
      symbTag[0][outerTag][0][validAttrTag][0]["se:SvgParameter"];
  }

  var nrOfCssTags = Object.keys(allCssArray).length;
  var j;
  for (j = 0; j < nrOfCssTags; j++) {
    //for all cssParameters
...
function getObjFromDiffAttr(tagName, type, symbTag, file) {
  ...
  } else if (tagName === "se:Fill") {
    //some fill-attributes are defined differently than the rest
...
function getLabelObj(tagName, type, symbTag, obj) {
  ...
  obj[convertedTagName] =
    "{" + symbTag[0].Label[0]["ogc:PropertyName"][0] + "}";
  return obj;
}
...
function getGeometryObj(symbTag, obj) {
  if (symbTag[0].Geometry[0]["ogc:Function"][0].$.name === "vertices") {
    ...
    console.log(
      "Cannot convert attribute value: " +
        symbTag[0].Geometry[0]["ogc:Function"][0].$.name +
        ", for tag Geometry"
    );
  }
  return obj;
...
function getGraphicObj(file, symbTag, type, obj) {
  ...
  try {
    fillColor =
      symbTag[0]["se:Graphic"][0]["se:Mark"][0]["se:Fill"][0][
        "se:SvgParameter"
      ][0]["_"];
...
function convert_css_parameter(cssTag, ValidAttrTag, type, outerTag) {
  ...
  try {
    ...
    if (
      DIFF_ATTR.indexOf(cssName) > -1 &&
      !regInt.test(cssTag["_"]) &&
      !regDouble.test(cssTag["_"]) &&
      !regLetters.test(cssColorValue) &&
      !regNumbers.test(cssColorValue)
    ) {
      //Check if different type of attribute
...
function convertCssName(cssName, validAttrTag, type, outerTag) {
  ...
  if (
    ...
    (cssName === "opacity" && validAttrTag === "Fill")
  ) {
    if (outerTag === undefined) {
      ...
      var newName =
        CONVERT_ATTR_NAME[
          type + "-" + outerTag + "-" + validAttrTag + "-" + cssName
        ];
      if (newName === undefined) {
        console.log(
          "could not convert the attribute name: " +
            type +
            "-" +
            outerTag +
            "-" +
...
function getPaintAndLayoutAttr(cssObj) {
  ...
  var layout = {};
  var i;
  for (i = 0; i < Object.keys(cssObj).length; i++) {
    // for all in cssObj
    ...
    if (PAINT_ATTR.indexOf(key) > -1) {
      ...
      console.log(
        "The css-key: " + key + ", is not a valid paint or layout attribute"
      );
    }
  }
  return [paint, layout];
